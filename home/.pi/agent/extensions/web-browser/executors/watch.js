...
import { readActiveTab, writeActiveTab, clearActiveTab } from "./active-tab.js";
...
async function main() {
  ...
  cdp.on("Target.targetDestroyed", (params) => {
    ...
    // Clear active tab state if the destroyed tab was the active one
    const activeState = readActiveTab();
    if (activeState?.targetId === targetId) {
      clearActiveTab();
    }
  });
  ...
  cdp.on("Target.targetInfoChanged", (params) => {
    ...
    if (state) {
      ...
      // Update active tab state if this is the tracked tab
      const activeState = readActiveTab();
      if (activeState?.targetId === info.targetId && info.url) {
        writeActiveTab(info.targetId, info.url);
      }
    }
